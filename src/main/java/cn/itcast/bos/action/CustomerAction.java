package cn.itcast.bos.action;

import java.util.HashMap;
import java.util.Map;
import java.util.UUID;
import java.util.concurrent.TimeUnit;

import javax.jms.JMSException;
import javax.jms.MapMessage;
import javax.jms.Message;
import javax.jms.Session;

import org.apache.struts2.convention.annotation.Action;
import org.apache.struts2.convention.annotation.Result;
import org.apache.struts2.convention.annotation.Results;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.redis.core.RedisTemplate;
import org.springframework.jms.core.JmsTemplate;
import org.springframework.jms.core.MessageCreator;

import com.opensymphony.xwork2.ActionContext;

import cn.itcast.bos.utils.CheckCodeUtils;
import cn.itcast.bos.utils.action.BaseAction;
import cn.itcast.crm.service.Customer;
import cn.itcast.crm.service.CustomerServiceImp;

@Results({ @Result(name = "toSuccess", type = "redirect", location = "/signup-success.html"),
		@Result(name = "toRegist", type = "redirect", location = "/signup.html"),
		@Result(name = "toLogin", type = "redirect", location = "/login.html"),
		@Result(name = "toIndex", type = "redirect", location = "/index.html"),
		@Result(name = "error", location = "/error.jsp") })
public class CustomerAction extends BaseAction<Customer> {
	@Autowired
	private CustomerServiceImp cs;
	//发送验证码的原因
	private String checkCodeType;
	// 发送验证码短信
	@Action("CustomerAction_sendCheckCode")
	public String sendCheckCode() throws Exception {
		// 0 创建一个用于json提示信息的map
		Map map = new HashMap();
		if (checkCodeType.equals("regist")) { //当前是发送注册验证码,判断手机号是否重复
			// 1 调用ws.检查手机号是否重复
			Customer existCustomer = cs.findByTelephone(model.getTelephone());
			if (existCustomer != null) {
				// 2 重复 => 将错误信息发送到页面提示
				// {type:error,msg:手机号已经被注册!}
				map.put("type", "error");
				map.put("msg", "手机号已经被注册!");
				map2Client(map);
				return null;
			} 
		}
		// 3 生成随机验证码
		String checkCode = CheckCodeUtils.getCheckCode(6);
		// 4 将验证码发送到手机
		// SmsUtils.sendSms(model.getTelephone(), checkCode);
		System.out.println(checkCode);
		// 5 将验证码存入session.
		ActionContext.getContext().getSession().put(model.getTelephone(), checkCode);
		// 6 向前台发送成功的提示信息
		map.put("type", "success");
		map.put("msg", "ok");
		map2Client(map);

		return null;
	}

	// 校验邮箱是否重复
	@Action("CustomerAction_checkMail")
	public String checkMail() throws Exception {
		// 0 创建一个用于json提示信息的map
		Map map = new HashMap();
		// 1 调用ws.根据邮箱查询客户
		Customer existCustomer = cs.findByEmail(model.getEmail());
		if (existCustomer != null) {
			// 2 重复 => 将错误信息发送到页面提示
			// {type:error,msg:手机号已经被注册!}
			map.put("type", "error");
			map.put("msg", "邮箱已被注册!");
			map2Client(map);
			return null;
		}
		// 3 向前台发送成功的提示信息
		map.put("type", "success");
		map.put("msg", "ok");
		map2Client(map);

		return null;
	}

	// 接收表单提交的验证码
	private String checkCode;

	@Autowired
	private RedisTemplate rt;
	@Autowired
	private JmsTemplate jt;

	// 注册
	@Action("CustomerAction_regist")
	public String regist() throws Exception {
		// 1 校验手机验证码
		// 取出session中的验证码
		String checkCodeSession = (String) ActionContext.getContext().getSession().get(model.getTelephone());
		// 比对
		if (checkCode.equals(checkCodeSession)) {
			// 发送激活邮件
			// --------------------------------------------------------------------------------
			// 1 生成激活码
		final	String activeCode = UUID.randomUUID().toString();
			// 2 发送激活邮件
		/*	MailUtils.sendMail("bos物流邮箱激活",
					"<a href='http://localhost:8888/bos_fore/CustomerAction_active.action?activeCode=" + activeCode
							+ "&email=" + model.getEmail() + "' >点我激活</a>",
					model.getEmail());*/
			//发送消息
			jt.send("bos_fore.mail", new MessageCreator() {
				@Override
				public Message createMessage(Session session) throws JMSException {
					MapMessage message = session.createMapMessage();
					message.setString("title", "bos物流邮箱激活");
					
					message.setString("content", "<a href='http://localhost:8888/bos_fore/CustomerAction_active.action?activeCode=" + activeCode
							+ "&email=" + model.getEmail() + "' >点我激活</a>");
					
					message.setString("to", model.getEmail());
					
					return message;
				}
			});
			// 3 存入redis
			rt.opsForValue().set(model.getEmail(), activeCode, 1, TimeUnit.DAYS);
			// --------------------------------------------------------------------------------
			// 指定客户为未激活状态
			model.setType(0);
			// 调用ws保存客户
			cs.save(model);
			// 3 成功=>重定向到注册成功页面
			return "toSuccess";
		} else {
			// 2 失败=>重定向到注册页面
			return "toRegist";
		}
	}

	// 接收邮箱激活码
	private String activeCode;

	// 激活邮箱
	@Action("CustomerAction_active")
	public String active() throws Exception {
		// 1 接收参数中的邮箱|激活码
		// 2 根据邮箱从redis中取出激活码,并移除
		String activeCode2 = (String) rt.opsForValue().get(model.getEmail());
		// 移除redis激活码
		rt.opsForValue().set(model.getEmail(), "", 1, TimeUnit.MICROSECONDS);
		// 3 比对激活码是否一致
		if (activeCode2 == null || !activeCode2.equals(activeCode)) {
			// 不一致=>转发到错误页面提示,激活码已过期或已经激活
			ActionContext.getContext().put("msg", "激活码已过期或已经激活!");
			return "error";
		}
		// 一致=>调用ws修改激活状态
		cs.active(model.getEmail());

		return "toLogin";
	}

	// 接收图片验证码
	private String validateCode;
	// 登陆方式标识
	private Integer article;

	// 登陆
	@Action("CustomerAction_login")
	public String login() throws Exception {
		// 0 判断当前登陆方式
		if (article == 0) {
			// 邮箱登陆
			return loginByEmailAndPassword();

		} else {
			// 手机号登陆(同学们的作业)
			return loginByTelephone();
		}

	}

	private String loginByTelephone() {
		//1.校验验证码,取出session中的验证码
		String checkCodeSession = (String) ActionContext.getContext().getSession().get(model.getTelephone());
		// 比对
		if (!checkCode.equals(checkCodeSession)) {
			//失败=>提示
			ActionContext.getContext().put("msg", "手机验证码错误!");
			return "error"; }
		//2.根据手机号获得客户
		Customer c = cs.findByTelephone(model.getTelephone());
			//获得不到=>请先注册再登陆
		if(c==null){
			// 未取出=>用户名或密码不正确
			ActionContext.getContext().put("msg", "手机号还未注册!");
			return "error";  }
		//3.校验激活状态
		if (c.getType() != 1) {
			// 未激活=>请先激活再登陆
			ActionContext.getContext().put("msg", "请先激活再登陆!");
			return "error"; }
		// 4.将客户放入session.作为登陆标识.
		ActionContext.getContext().getSession().put("customer", c);
		// 5.重定向到首页
		return "toIndex";
	}

	private String loginByEmailAndPassword() {
		// 1. 校验图片验证码是否正确
		String validateCode2 = (String) ActionContext.getContext().getSession().get("validateCode");
		// 表单验证码与session验证码比对
		if (validateCode2 == null || !validateCode2.equalsIgnoreCase(validateCode)) {
			// 验证码比对失败
			ActionContext.getContext().put("msg", "验证码错误!");
			return "error"; }
		// 2.调用Ws根据账户&密码取出客户对象
		Customer c = cs.findByEmailAndPassword(model.getEmail(), model.getPassword());
		if (c == null) {
			// 未取出=>用户名或密码不正确
			ActionContext.getContext().put("msg", "用户名或密码错误!");
			return "error"; }
		// 3.判断客户是否已经激活
		if (c.getType() != 1) {
			// 未激活=>请先激活再登陆
			ActionContext.getContext().put("msg", "请先激活再登陆!");
			return "error"; }
		// 4.将客户放入session.作为登陆标识.
		ActionContext.getContext().getSession().put("customer", c);
		// 5.重定向到首页
		return "toIndex";
	}

	public String getCheckCode() {
		return checkCode;
	}

	public void setCheckCode(String checkCode) {
		this.checkCode = checkCode;
	}

	public String getActiveCode() {
		return activeCode;
	}

	public void setActiveCode(String activeCode) {
		this.activeCode = activeCode;
	}

	public String getValidateCode() {
		return validateCode;
	}

	public void setValidateCode(String validateCode) {
		this.validateCode = validateCode;
	}

	public Integer getArticle() {
		return article;
	}

	public void setArticle(Integer article) {
		this.article = article;
	}

	public String getCheckCodeType() {
		return checkCodeType;
	}

	public void setCheckCodeType(String checkCodeType) {
		this.checkCodeType = checkCodeType;
	}

}
